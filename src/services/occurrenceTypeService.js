...
export async function getAll() {
  try {
    const result = await index.get("/occtypes/all");
...
export async function get(currentPage, pageSize) {
  try {
    const result = await index.get("/occtypes", {
      params: {
        currentPage,
        pageSize,
      },
...
export async function create(dto) {
  try {
    const result = await index.post("/occtype", {
      name: dto.name,
    });
...
export async function del(dto) {
  try {
    const result = await index.delete(`/occtype/${dto.id}`);
...
export async function updt(formData, currentDepartment) {
  try {
    const result = await index.put(`/occtype/${currentDepartment.id}`, {
      name: formData.name,
    });
